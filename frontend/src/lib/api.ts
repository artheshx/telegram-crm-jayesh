...
export const reconnectAccount = (id: number) =>
  api.post(`/accounts/${id}/reconnect`).then((r) => r.data)
...
export const stopScraping = (jobId: number) =>
  api.post(`/scraper/stop/${jobId}`).then((r) => r.data)

export const getScrapeHistory = () =>
  api.get('/scraper/history').then((r) => r.data)
